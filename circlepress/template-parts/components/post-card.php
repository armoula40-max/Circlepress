...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'cp-card' ); ?>>
	<a class="cp-card__media" href="<?php the_permalink(); ?>" aria-label="<?php the_title_attribute(); ?>">
		...
		<?php else : ?>
			<span style="display:flex;align-items:center;justify-content:center;height:100%;min-height:180px;font-size:3rem;background:linear-gradient(135deg,var(--cp-surface-2),var(--cp-border))"><?php echo esc_html( circlepress_get_niche( circlepress_get_current_niche() )['icon'] ); ?></span>
		<?php endif; ?>
